...
dnl Check for libcelia and set CELIA_CFLAGS and CELIA_LIBS
dnl appropriately.

AC_DEFUN([CELIA_CHECK],
[

AC_MSG_CHECKING(for libcelia)

AC_ARG_WITH(
  celia-include,
  AC_HELP_STRING(
    [--with-celia-include=DIR],
    [look for the header celia.h in DIR rather than the default search path]),
  [CELIA_CFLAGS="-I$withval"], [CELIA_CFLAGS=""])

AC_ARG_WITH(
  celia-lib,
  AC_HELP_STRING(
    [--with-celia-lib=DIR],
    [look for libcelia.a in DIR rather than the default search path]),
  [
    case $withval in
      /* ) true;;
      *  ) AC_MSG_ERROR([

You must specify an absolute path for --with-celia-lib.
]) ;;
    esac
    CELIA_LIBS="-L$withval -lcelia"
  ], [CELIA_LIBS="-lcelia"])
...
AC_TRY_LINK(
  ...
  [
    AC_MSG_RESULT(found)
    AC_SUBST(CELIA_CFLAGS)
    AC_SUBST(CELIA_LIBS)
    AC_DEFINE(HAVE_CELIA,1,[Defined if libcelia is installed])
  ],
  [
    AC_MSG_RESULT(not found)
    AC_MSG_ERROR([

The library libcelia was not found on your system! Please obtain it
from

  http://acsc.csl.sri.com/cpabe/

and install it before trying again. If libcelia is already
installed in a non-standard location, try again with

  ./configure --with-celia-include=<path> --with-celia-lib=<path>

If you already specified those arguments, double check that celia.h
can be found in the first path and libcelia.a can be found in the
second.
